...
function sessionMatchesSpan(session, spanId) {
  if (!session || !spanId) return false;
  return session.spanId === spanId || (session.aliasSpanIds || []).includes(spanId);
}
...
function ensureSelectedSession(sessions) {
  ...
  const current = sessions.find((session) => sessionMatchesSpan(session, state.selectedSessionId));
  if (current) {
    state.selectedSessionId = current.spanId;
    return current;
  }
...
function buildLocateLogButton(logEntryId, className = "session-flow-locate") {
  const button = document.createElement("button");
  button.type = "button";
  button.className = className;
  button.dataset.logEntryId = logEntryId || "";
  button.textContent = "定位日志";
  button.title = "定位并展开来源日志";
  button.addEventListener("click", (event) => {
    event.stopPropagation();
    focusLogEntry(button.dataset.logEntryId, { expandRaw: true });
  });
  return button;
}

function patchLocateLogButton(container, logEntryId, className = "session-flow-locate") {
  if (!container) return;
  ...
  if (button && button.dataset.logEntryId === logEntryId) return;
  const next = buildLocateLogButton(logEntryId, className);
  if (button) {
...
function reasoningTokenCount(session) {
  const explicit = Number(session?.reasoningTokens);
  if (Number.isFinite(explicit) && explicit > 0) return explicit;
  const usage = session?.tokenUsage || {};
  return ["reasoning_tokens", "reasoning", "reasoningTokens"].reduce((sum, key) => {
    const value = Number(usage[key]);
    return Number.isFinite(value) ? sum + value : sum;
  }, 0);
}

function buildFlowAside(step) {
  const aside = document.createElement("div");
  aside.className = "session-flow-aside";
  aside.appendChild(buildFlowBadge(step.badgeLabel, step.badgeKind, step.liveClass));
  if (step.logEntryId) {
    aside.appendChild(buildLocateLogButton(step.logEntryId));
  }
  return aside;
}

function buildReasoningDetails(step) {
  if (state.privacyMode) {
    const hidden = document.createElement("p");
    hidden.className = "session-flow-body session-reasoning-hidden";
    hidden.textContent = "隐私模式已隐藏模型思考内容";
    return hidden;
  }
  const details = document.createElement("details");
  details.className = "session-flow-body session-reasoning-details";
  if (step.reasoningDetailKey) bindDetailsState(details, step.reasoningDetailKey);
...
function patchFlowBody(row, step) {
  ...
  if (needsReasoning) {
    if (state.privacyMode) {
      body.textContent = "隐私模式已隐藏模型思考内容";
      return;
    }
    const summary = body.querySelector("summary");
    if (summary) summary.textContent = "模型思考";
  ...
  body.classList.toggle("is-running", step.liveClass === "is-running");
  body.classList.toggle("is-generating", step.liveClass === "is-generating");
}
...
function buildFlowStep(step, index) {
  ...
  const aside = buildFlowAside(step);
  head.append(title, aside);

  const body = buildFlowBody(step);
...
function updateFlowStep(row, step, index) {
  ...
  row.classList.toggle("reasoning", step.kind === "reasoning");
  ...
  if (aside) {
    ...
    if (existingBadge) {
      ...
      aside.prepend(buildFlowBadge(step.badgeLabel, step.badgeKind, step.liveClass));
    }
    patchLocateLogButton(aside, step.logEntryId || "");
  }
...
function buildSessionJourney(session, insights) {
  ...
  const messageInEvent = events.find((event) => event.type === "message_in");
  ...
  const reasoningTokens = reasoningTokenCount(session);
  const live = sessionIsLive(session);
  ...
  function eventLogEntryId(event) {
    return event?.evidence?.logEntryId || "";
  }
  ...
  steps.push({
    ...
    state: "done",
    kind: "message",
    logEntryId: eventLogEntryId(messageInEvent),
  });
  ...
  if (personaEvent || session.personaId) {
    ...
    steps.push({
      ...
      state: "done",
      kind: "context",
      logEntryId: eventLogEntryId(personaEvent),
    });
  }
  ...
  tools.forEach((tool, idx) => {
    ...
    steps.push({
      ...
      kind: "tool",
      logEntryId: tool.resultLogEntryId || tool.evidenceLogEntryId || "",
    });
  ...
  if (session.status === "complete") {
    ...
    steps.push({
      ...
      state: "done",
      kind: "request",
      logEntryId: eventLogEntryId(messageOutEvent),
    });
    ...
    steps.push({
      ...
      state: "error",
      kind: "request",
      logEntryId: eventLogEntryId(errorEvent),
    });
  }

  if (session.reasoningContent && (session.status === "complete" || session.displayStatus === "empty")) {
    steps.push({
      key: "reasoning",
      title: "模型思考",
      body: state.privacyMode ? "隐私模式已隐藏模型思考内容" : "模型思考",
      reasoningContent: session.reasoningContent,
      reasoningDetailKey: detailKey("session-reasoning", session.spanId, session.reasoningLogEntryId || session.reasoningTs || ""),
      timeLabel: session.reasoningTs ? formatTime(session.reasoningTs) : "--",
      meta: joinMeta([
        [session.providerId, session.model].filter(Boolean).join(" / "),
        reasoningTokens ? `思考 ${formatNumber(reasoningTokens)} Token` : "",
      ]),
      badgeLabel: "已记录",
      badgeKind: "debug",
      state: "done",
