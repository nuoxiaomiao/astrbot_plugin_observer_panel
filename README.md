...
- `access_token`: 面板访问令牌。监听 `0.0.0.0` 时，建议必须配置。
- `refresh_interval_seconds`: 前端自动刷新间隔，默认 `5` 秒。
- `astrbot.logs_dir`: AstrBot 日志目录，默认 `data/logs`。
- `astrbot.log_files`: AstrBot 日志文件名或绝对路径列表。
- `astrbot.tail_lines`: 每个日志文件读取的最大行数。
- `astrbot.tail_bytes`: 每个日志文件读取的最大字节数。
- `ui.log_page_size`: 原始日志每页行数，默认 `80`。
- `ui.privacy_mode`: 隐藏消息内容、工具参数、工具结果和原始日志正文。
...
- 未配置 `access_token` 且监听 `0.0.0.0` 时，远程请求会被整体拒绝。
...
2. 日志为空：
   ...
   - 确认 AstrBot 进程对日志文件有读权限

3. 远程访问失败：
   - 监听 `0.0.0.0` 时请配置 `access_token`
